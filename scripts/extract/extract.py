...
def extract_data(schema):
    spark=SparkSession.builder \
        .appName('extractor') \
        .getOrCreate()
    

    data1 = spark.read.csv('/home/basel/main/Grad_proj/data/raw/salaries_2.csv', header=True, schema=schema)
    data2 = spark.read.csv('/home/basel/main/Grad_proj/data/raw/jobs_in_data.csv', header=True, inferSchema=True)
    data3 = spark.read.csv('/home/basel/main/Grad_proj/data/raw/jobs.csv', header=True, inferSchema=True)
    data2 = data2.select(
    "work_year",
    "experience_level",
    "employment_type",
    ...
    df_parquet= spark.read.parquet('/home/basel/main/Grad_proj/data/raw/data.parquet')    
    merged_dataset = data1.union(data2).union(data3).union(df_parquet)
    merged_dataset.coalesce(1).write.parquet("/home/basel/main/Grad_proj/data/processed", mode='overwrite')
